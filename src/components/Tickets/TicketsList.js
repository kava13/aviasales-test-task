...
class TicketsList extends React.Component {
  ...
  createDateAsUTC = (date) => {
    const [ day, month, year, hours, minutes] = [date.getUTCDate(), date.getUTCMonth(), date.getUTCFullYear(),  date.getUTCHours(), date.getUTCMinutes()];
    return new Date (year, month, day, hours, minutes);
  }

  formatDate = (date, separator) => {
    return separator? format(date, `HH${separator}mm`) : format(date, `HHч mmм`);
  }
  ...
  renderTicket = () => (item, index) => {
    ...
    console.log('TICKET TRAVEL DURATION', item.segments.reduce((acc, segment) => acc + segment.duration, 0));
    ...
    <li className="tickets-list__item">
      ...
      {item.segments.map( (segment) => {
        ...
        const [durationHours, durationMinutes] = this.calculateDuration(segment.duration);
        const dateDuration = new Date(2020, 0, 1, durationHours, durationMinutes);
        ...
        return (
          <div className="tickets-item__info">
            <p>
              ...
              <span>{this.formatDate(dateOrig, ':')} - {this.formatDate(dateDest, ':')}</span>
            </p>
            <p>
              <span>В ПУТИ</span>
